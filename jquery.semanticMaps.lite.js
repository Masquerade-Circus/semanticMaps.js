            ...
            function initSemanticMaps() {
                function addSearchBox(map, google) {
                    ...
                    google.maps.event.addListener(searchBox, 'places_changed', function () {
                        var places = searchBox.getPlaces(), i, bounds, place_marker, place, options, contenido;
                        if (places.length > 0) {
                            ...
                            for (i = 0; place = places[i]; i++) {
                                ...
                                if (place.photos) {
                                    options.icon = place.photos[0].getUrl({'maxWidth': 35, 'maxHeight': 35});
                                }
                                ...
                                contenido = '';
                                if (place.photos) {
                                    contenido += '<img src="' + place.photos[0].getUrl({'maxWidth': 150, 'maxHeight': 100}) + '" style="display: block; float: left; margin: 0 1em 1em 0">';
                                }
                                contenido += '<b>Nombre:</b> ' + place.name + '<br><b>Dirección:</b> ' + place.formatted_address + '<br>';
                                if (place.types && place.types.length > 0) {
                                    contenido += '<b>Tipo de establecimiento:</b> ' + place.types.join(',') + '<br>';
                                }
                                if (place.website) {
                                    contenido += '<b>Sitio web:</b> ' + place.website + '<br>';
                                }
                                if (place.formatted_phone_number) {
                                    contenido += '<b>Teléfono:</b> ' + place.formatted_phone_number + '<br>';
                                }
                                ...
                                if (place.rating) {
                                    contenido += '<b>Rating:</b> ' + place.rating + '<br>';
                                }
